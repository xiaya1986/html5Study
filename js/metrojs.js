		...
		init: function (options) {
			...
			if (stgs.useHardwareAccel == true) {
				if (stgs.useModernizr == false) {
					//determine if the browser supports the neccessary accellerated features
			...
			return $(this).each(function () {
				...
				if (tdata.mode == 'flip-list') {
					...
					if (canFlip3d && tdata.useHwAccel) {
						...
						var fCss = (tdata.direction == "vertical") ?
							   ...
							   { height: height + 'px', width: '0px !important', marginLeft: margin + 'px', opacity: '0' };
				...
				$this.slide = function () {
					...
					if (canTransition && tdata.useHwAccel) {
						...
						$firstContainer.animate(uCss, tdata.speed, function () {
							...
							privMethods.handleSlide(clojIsReversed, $firstContainer, swapFrontSource, stgs, staticCount);
							if (!clojIsReversed && swapFrontSource) {
								staticCount += 1;
								if (staticCount >= stgs.frontImages.length)
									staticCount = 0;
				...
				$this.flip = function () {
					...
					if (canFlip3d && tdata.useHwAccel) {
						...
						if (isReversed) {
							...
							$firstContainer.stop().animate(upCss, { duration: tdata.speed }, function () {
								stgs.animationComplete(tdata, $scndContainer, $firstContainer);
							});
							window.setTimeout(function () {
							...
							window.setTimeout(function () {
								$firstContainer.stop().animate(dwnCss2, { duration: tdata.speed }, function () {
									stgs.animationComplete(tdata, $firstContainer, $scndContainer);
								});
								if (swapBackSource) {
									var isRandom = stgs.backIsRandom;
				...
				$this.flipList = function () {
					...
					//                    if (swapFrontSource) {
					//                        for (var i = 0; i <= stgs.frontImages.length - 1; i++) {
					//                            fBag[i] = i;
					//                        }
					//                    }
					//                    if (swapBackSource) {
					//                        for (var i = 0; i <= stgs.backImages.length - 1; i++) {
					//                            bBag[i] = i;
					//                        }
					//                    }
